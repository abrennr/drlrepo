...
class BaseIngestObject:
    ...
    def __init__(self, bag_path):
        ...
        m = ingest_utils.get_item_metadata(bag_path)
        self.item_id = ingest_utils.get_item_id(m)
        ...
        self.mods_label = ingest_utils.get_mods_name(m)
        ...
        self.dc_label = ingest_utils.get_dc_name(m)
        self.dc_path = os.path.join(pathroot, self.dc_label) 
        self.marcxml_label = ingest_utils.get_marcxml_name(m)
        if self.marcxml_label:
            self.marxml_path = os.path.join(pathroot, self.marcxml_label) 
        self.mets_label = ingest_utils.get_mets_name(m)
        if self.mets_label:
            self.mets_path = os.path.join(pathroot, self.mets_label) 
        self.target_label = ingest_utils.get_target_name(m)
        if self.target_label:
            self.target_path = os.path.join(pathroot, self.target_label) 
        item_thumbnail_source = os.path.join(pathroot, ingest_utils.get_item_thumbnail_source(m))
        ...
        if len(master_files) > 1:
            ...
            ocr_path = os.path.join(pathroot, ingest_utils.get_ocr_name(m))
            ...
            self.jp2_label = ingest_utils.get_jp2_name(m, self.obj_label)
